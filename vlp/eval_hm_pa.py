...
from vlp.loader_utils import batch_list_to_batch_tensors, \
    batch_list_to_batch_tensors_paired, \
    batch_list_to_batch_tensors_paired_eval
...
def validate_set(epoch_num, eval_model_recover_path, args, logger, validation_set='dev'):
    if args.enable_butd:
        ...
        args.region_det_file_prefix = os.path.join(args.image_root, args.region_det_file_prefix) \
            if args.dataset in ('cc', 'coco', 'hm', 'hm-paired') and args.region_det_file_prefix != '' else ''
    ...
    for model_recover_path in glob.glob(eval_model_recover_path.strip()):
        ...
        model = BertForPreTrainingLossMask.from_pretrained(
            ...
            len_vis_input=args.len_vis_input, tasks='hm-paired-attn')
        del model_recover
        ...
        print('Epoch {}: start the HM with paired attention evaluation...'.format(epoch_num))
                ...
                for instance in buf:
                    for proc in bi_uni_pipeline:
                        if not validation_set == 'test':
                            ...
                            instances.append((instance_left[:-1], instance_right[:-1], label_tk))
                        else:
                            src_tk, text_tk, im2text_tk = instance
                            instance_left = proc((src_tk, text_tk))
